...
(deftest win-predicate-test
  (testing "Emitted predicates behave as expected"
    (are [expected required-numbers called-numbers]
      (let [pred (sut/->win-predicate required-numbers)]
...
(deftest winning-predicates-test
  (testing "Getting all numbers in a row or column should be considered a win"
    (are [expected called-numbers]
      (= expected (sut/win? called-numbers
                    {:winning-conditions (sut/generate-winning-predicates [[1 2 3]
                                                                           [4 5 6]
                                                                           [7 8 9]])}))
  ...
  (testing "Against real data"
    (are [expected called-numbers]
      (= expected (count (filter (partial sut/win? called-numbers)
                           sut/bingo-boards)))
...
(deftest example-winner-test
  (is (= 4512 (:score (first (sut/score-boards 1 example-bingo-numbers example-bingo-boards)))))
  (is (= 1924 (:score (last (sut/score-boards 1 example-bingo-numbers example-bingo-boards))))))
